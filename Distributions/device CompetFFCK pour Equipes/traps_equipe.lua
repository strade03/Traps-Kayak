...
-- Information : Numero de Version, Nom, Interface
function device.GetInformation()
	return { version = 1.0, name = 'Serveur TRAPS_Equipe', class = 'network' };
end	
...
function device.OnConfiguration(node)
	...
	local dlg = wnd.CreateDialog({
		...
		label = "Configuration Gestion des Pena",
		width = 300,
		height = 200
	});
	
	dlg:LoadTemplateXML({ 
		xml = './device/traps_equipe.xml',
...
function ReadPacket(cb)
	...
	if #tab >= 5 and tab[1]=="penalty" then
		...
		-- Alert("TRAPS: dossard "..bib.." porte "..gate.." penalite "..penalty.." embarcation "..boat)
...
function AddPenalty(bib, gate, embarcation, penalty)
	...
	local boat = ((gate - 1) % 3) + 1
	local porte = math.floor((gate - 1) / 3) + 1	
	Alert("TRAPS: dossard "..bib.." porte= "..porte.." penalite "..penalty.." embarcation "..boat)
	app.SendNotify('<penalty_add>',
		{ bib = bib, gate = porte, embarcation = boat, penalty = penalty  }
	);
